...
avoi_rate   = 0.01                           # Avoid rate = how much the Avoidance vs the flocking direction weighs desired direction
...
while t <= max_time:
    ...
    for locust in locusts:
        ...
        # region -- only LC +pos
        print(np.sum(looming_cue_unnorm))
        #locust[:, 1] = - np.sum(looming_cue) * 
        #endregion
